...
class CommandLine {
    ...
    run() {
        ...
        platform = this.commander.choice('請輸入需要變更套件來源的套件管理平台名稱：', ['composer', 'npm'], 0);
        const config = package_util_1.default.getConfig();
        const repositoriePlatforms = Object.keys(config.platforms);
        packageName = this.commander.choice(`請輸入 ${platform} 需要變更套件來源的套件名稱：`, repositoriePlatforms, 0);
        ...
        switch (platform) {
            ...
            case 'npm': {
                ...
                this.commander.line(`準備替換 ${platform} 套件管理平台中的 ${packageName} 套件來源，目前的套件來源是 "${currentPackageType}"`);
                repositorieSources = repositorieSources.filter((item) => item.source !== currentPackageType);
                break;
            }
            default:
        ...
        source = this.commander.choice(`請輸入 ${platform} 套件管理平台中的 ${packageName} 套件應改為哪一種套件來源：`, repositorieSources, 0);
